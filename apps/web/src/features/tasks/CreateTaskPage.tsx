...
export function CreateTaskPage({ onSubmit, onCancel }: { onSubmit: (payload: TaskCreateInput, files: File[]) => void | Promise<void>; onCancel: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (goal === "edit_only") {
        await onSubmit({ goal, market: "ID", platform: "tiktok", editInstructions, allowedOperations: operations }, editFiles);
        return;
      }
